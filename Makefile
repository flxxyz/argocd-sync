...
BASE_PACKAGE_NAME  = github.com/flxxyz/argocd-sync
...
.PHONY: build
build:
	CGO_ENABLED=0 go build $(LDFLAGS) -installsuffix cgo -o dist/argocd-sync main.go

build-for-container:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build $(LDFLAGS) -a -installsuffix cgo -o dist/argocd-sync-linux main.go
...
release: build-for-container
	...
	docker tag argocd-sync:latest flxxyz/argocd-sync:$(GIT_VERSION)
	docker push flxxyz/argocd-sync:$(GIT_VERSION)
