...
BASE_PACKAGE_NAME  = github.com/flxxyz/argocd-sync-app
...
.PHONY: build
build:
	CGO_ENABLED=0 go build $(LDFLAGS) -installsuffix cgo -o dist/argocd-sync-app main.go

build-for-container:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build $(LDFLAGS) -a -installsuffix cgo -o dist/argocd-sync-app-linux main.go
...
release: build-for-container
	...
	docker tag argocd-sync-app:latest flxxyz/argocd-sync-app:$(GIT_VERSION)
	docker push flxxyz/argocd-sync-app:$(GIT_VERSION)
